...
:::tip[Handy Tip]
That's why in ao-starter-kit the `process.lua` file is broken down into `process_lib.lua` and the entry point file only contains the `Handlers` and the global variables.
:::
